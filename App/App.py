...
def run():
    ...
    # Create the DB (sqlite handles it via file)
    ...
    if choice == 'User':
        ...
        st.markdown('''<h5 style='text-align: left; color: #021659;'> Paste Job Description For ATS Compatibility Score (Optional) </h5>''',unsafe_allow_html=True)
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                if job_description:
                    st.subheader("**ATS Score vs Job Description 📊**")
                    import spacy
                    from pyresparser import utils as res_utils
                    try:
                        nlp_model = spacy.load('en_core_web_sm')
                        nlp_text = nlp_model(job_description)
                        noun_chunks = list(nlp_text.noun_chunks)
                        jd_skills = res_utils.extract_skills(nlp_text, noun_chunks)
                        
                        if jd_skills and resume_data.get('skills'):
                            jd_skills_lower = [s.lower() for s in jd_skills]
                            resume_skills_lower = [s.lower() for s in resume_data['skills']]
                            
                            match_count = sum(1 for skill in jd_skills_lower if skill in resume_skills_lower)
                            ats_score = (match_count / len(jd_skills_lower)) * 100
                            
                            ats_bar = st.progress(0)
                            for percent_complete in range(int(ats_score)):
                                time.sleep(0.01)
                                ats_bar.progress(percent_complete + 1)
                                
                            st.success(f"** Your ATS Score is: {round(ats_score, 2)}% **")
                            
                            missing_skills = list(set([s.capitalize() for s in jd_skills_lower]) - set([s.capitalize() for s in resume_skills_lower]))
                            if missing_skills:
                                st.warning(f"**Missing Skills from JD:** {', '.join(missing_skills)}")
                        else:
                            st.warning("Could not extract enough skills from Job Description to calculate ATS score.")
